...
class BaseSpider:
    ...
    def __init__(self, init_data):
        ...
        """
        self.selector = {"tag": init_data["tag"], "classes": init_data["classes"]}
    ...
    def is_accept_url(url):
        ...
        return (
            "/vi/viec-lam/" in url
            and not any(_ in url for _ in disallow_path)
            and url.count("/") == 5
        )
    ...
    def get_post_date(date_str):
        ...
        if match:
            ...
            if "ngày" in date_str:
                ...
                if time > 15:
                    return False
                return (now - datetime.timedelta(days=time)).strftime(
                    "%m/%d/%Y - %H:%M:%S"
                )
            elif "giờ" in date_str:
                return (now - datetime.timedelta(hours=time)).strftime(
                    "%m/%d/%Y - %H:%M:%S"
                )
            elif "phút" in date_str:
                return (now - datetime.timedelta(minutes=time)).strftime(
                    "%m/%d/%Y - %H:%M:%S"
                )
    ...
    def handle(self, base_url):
        ...
        item = soup.find(self.selector["tag"]["base"], self.selector["classes"]["base"])
        if not item:
            return {}
        ...
        for tag in atag:
            _url = tag["href"]
            if any(_ in _url for _ in extend_path) and _url not in crawled:
                crawled.append(_url)
                self.crawl(_url)
    ...
    def parse_address(self, item):
        ...
        return self.bsfind(item, tag, classes).replace("  ", "").replace("\n", "")

    def parse_post_date(self, item):
        tag = self.selector["tag"]["post_date"]
        classes = self.selector["classes"]["post_date"]
        return self.get_post_date(self.bsfind(item, tag, classes))
